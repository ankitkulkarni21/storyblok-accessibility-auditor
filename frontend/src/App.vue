<template>
  <div id="app">
    ...
    <div class="content-overlay">
      ...
      <p class="subtitle">Enter a Story Slug below to run an accessibility audit.</p>
      
      <form @submit.prevent="fetchAndAudit" class="input-form">
         <input type="text" v-model="storySlug" placeholder="Enter Story Slug (e.g., 'audit-test')" />
...
const fetchAndAudit = async () => {
    ...
    try {
        const { data } = await storyblokApi.get(`cdn/stories/${storySlug.value}`, {
            version: 'draft',
        });
        ...
        if (err.message && err.message.includes('404')) {
            error.value = `Story not found for slug: "${storySlug.value}". Please check the slug and ensure the story is Published.`;
            return;
        }

        if (err.code === 'ERR_NETWORK') {
            error.value = "Network Error: Could not connect to backend. Ensure Go server is running on the correct port.";
            return;
        }

        error.value = "Failed to run audit. Check the Storyblok token or server logs for details.";
